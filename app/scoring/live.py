...
def live_session(cfg: dict | None = None, pred: dict | None = None) -> dict:
    ...
    # A running read of how directional the 9:30->now session is (full-session
    # Kaufman ER, closes only). At/after the close this is the whole session and
    # matches the post-close labeler, so the live strip agrees with the stored grade.
